...
function setup(){
  ...
  title = createSprite(displayWidth/2 , 60, 20, 20);
  ...
  goal = createSprite(displayWidth/2 , displayHeight/2 , 250, 60);
  goal.addImage("goal", goalImg);
  goal.scale=0.5;

  replay = createSprite(displayWidth/2 +400, displayHeight/2 + 200, 250, 60);
  replay.addImage("replay", replayImg);
  replay.scale = 0.5;
  replay.visible = false;

  letsGo = createSprite(displayWidth/2 + 300, displayHeight/2 +300, 250, 60);
  letsGo.addImage("Go",goodToGo);
  letsGo.scale = 0.5;
  letsGo.visible = false;

  motherHen = createSprite(displayWidth/2, -200, 5,5);
  ...
  earth = createSprite(displayWidth/2, displayHeight/2, 20,20);
  earth.addImage("earth", earthImg);
  earth.scale = 2;
  earth.visible = false;

  happyEnd= createSprite(displayWidth/2, 150, 500, 50);
  happyEnd.addImage("happy", happyEndImg);
  happyEnd.visible = false;

  sadEnd= createSprite(displayWidth/2, displayHeight/2, 500, 50);
...
function draw(){
  ...
  if(gameState === START ){
    ...
    fill("deepPink")
    textSize(40)
    text("PRESS SPACE TO START!!", displayWidth/2 - 200, displayHeight-50)
  ...
  if(gameState === PLAY){
    ...
    if(level === 1 && score<10500){
      ...
      if((touches.length > 0 || keyDown("SPACE")) && score!=10500) {
        touches = [];
        shoot();
      }
    ...
    if(level===2 && score<30500){
      ...
      if((touches.length > 0 || keyDown("SPACE")) && score!=30500) {
        touches = [];
        shoot();
      }
      ...
      for(var k = 0; k<stones.length; k++){
        if(stones.get(k)!=undefined && (stones.get(k).x<=0 || stones.get(k).y>=displayHeight)){
    ...
    if(level === 3 && score<50000){
      ...
      if((touches.length > 0 || keyDown("SPACE")) && score!=50000) {
        touches = [];
        shoot();
      }
      ...
      for(var k = 0; k<birds3.length; k++){
        if(birds3.get(k)!=undefined && (birds3.get(k).y>=displayHeight)){
  ...
  if(gameState === END){
    ...
    if(score>=50000){
      spaceship.x = displayWidth/2 - 300;
      spaceship.y = displayHeight/2 + 200;
...
function level1(){
  if(frameCount%15 === 0){
    ...
    bird.scale = 0.5;
    bird.y = random(100,displayHeight - 50);
...
function level2(){
  if(frameCount%10 === 0){
    ...
    var rand = Math.round(random(1,2));
    if(rand ===1){
       stone.x = Math.round(random(200, displayWidth));
       stone.y = 0;
    }
    else if(rand === 2){
      stone.x = displayWidth;
      stone.y = Math.round(random(0, displayHeight));
    ...
    stone.lifetime = displayWidth/stone.velocityX;
    stones.add(stone);
...
function level3(){
  ...
  if(motherHen.y<displayHeight/2 - 100){ 
    motherHen.velocityY = 2;
  }
  else if(motherHen.y>displayHeight/2 - 100){
    motherHen.velocityY = 0;
  }
  else if(count >= 100 && motherHen.y<displayHeight/2 - 100){
    motherHen.velocityY = -2;
  }
  ...
  if(frameCount%20 === 0){
    ...
    if(rand===1){
      bird.x = 0;
      bird.y = Math.round(random(40, displayHeight-20));
      bird.velocityX = (8);
    }
    else if(rand===2){
      bird.x = displayWidth;
      ...
      bird.x = Math.round(random(50,displayWidth - 50));
      bird.y = 0;
      bird.velocityY = (8);
    }
    console.log(rand);
